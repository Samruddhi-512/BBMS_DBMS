...
const Dashboard = () => {
  ...
  const fetchFloorAnalytics = async (floorId) => {
    ...
    try {
      const { data } = await api.get(`/analytics/floor-stats/${floorId}`);
      setFloorStats(data);
    } catch (err) {
      console.error(err);
    }
            ...
            {searchResults.buildings?.length > 0 && (
              <div>
                ...
                {searchResults.buildings.map(b => (
                  <div key={b._id} className="search-item">{b.name}</div>
                ))}
              </div>
            )}
            ...
            {searchResults.employees?.length > 0 && (
              <div style={{ marginTop: '1rem' }}>
                ...
                {searchResults.employees.map(e => (
                  <div key={e._id} className="search-item">
                    {e.name} ({e.role})
                  </div>
                ))}
              </div>
        ...
        >
          ...
          {floors.map(f => (
            <option key={f._id} value={f._id}>
              Floor {f.floorNumber} - {f.buildingId?.name}
            </option>
          ))}
        </select>
